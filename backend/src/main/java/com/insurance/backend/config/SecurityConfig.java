...
@Configuration
public class SecurityConfig {   // removed @EnableWebSecurity – it's automatic in recent versions
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/", "/auth/login", "/auth/register").permitAll()
                .requestMatchers("/uploads/**").permitAll()
                .requestMatchers("/api/profile/**", "/admin/users/me").authenticated()
                .requestMatchers("/admin/register-admin", "/admin/register-surveyor").hasRole("ADMIN")
                .requestMatchers("/admin/**").hasRole("ADMIN")
                .requestMatchers("/surveyor/**").hasRole("SURVEYOR")
                .requestMatchers("/user/**").hasRole("USER")
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        CorsConfiguration config = new CorsConfiguration();
        config.setAllowedOriginPatterns(List.of("*"));   // or "https://vehicle-insurance-pi.vercel.app"
        config.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"));
        config.setAllowedHeaders(List.of("*"));
        config.setAllowCredentials(true);
        config.setMaxAge(3600L);
